...
const Notifications = () => {
    ...
    const NotificationCard = ({ notification }) => {
        ...
        return (
            <div 
                className={`group relative p-4 transition-all duration-300 border-b border-gray-100 hover:shadow-md hover:-translate-y-0.5 ${
                    !isRead ? 'bg-gradient-to-r from-green-50/80 to-emerald-50/50' : 'bg-white hover:bg-gray-50/50'
                            ...
                            <div className="flex gap-2">
                                {!isRead && (
                                    <button
                                        ...
                                        disabled={isMarking}
                                        className={`group/btn cursor-pointer flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg transition-all ${
                                            isMarking 
                                                ? 'bg-gray-100 text-gray-400 cursor-not-allowed' 
                                                : 'bg-main/10 text-main hover:bg-main hover:text-white hover:shadow-md'
                                        }`}
                                    >
                                        <i className={`fas fa-check text-xs ${isMarking ? 'animate-spin fa-spinner' : ''}`}></i>
                                        <span>{isMarking ? t("notifications.marking") || "جاري..." : t("notifications.markAsRead") || "مقروء"}</span>
                                    </button>
                                )}
                                <button
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-green-50/30 to-gray-50 py-6" dir={i18n.language === "ar" ? "rtl" : "ltr"}>
            {toast && (
                <div className={`fixed top-4 ${isRTL ? "left-4" : "right-4"} z-50 animate-slide-in`}>
                    <div className={`px-4 py-3 rounded-xl shadow-xl flex items-center gap-2 backdrop-blur-sm transform transition-all hover:scale-105 ${
                        toast.type === "success" 
                        ...
                        <div className="relative">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    {unreadCount > 0 && (
                                        <div className="relative">
                                            <div className="absolute inset-0 bg-gradient-to-r from-main to-green-600 blur-md opacity-30 rounded-full"></div>
                                            <div className="relative px-4 py-2 bg-gradient-to-r from-main to-green-600 text-white text-xs rounded-full font-bold shadow-md">
                    ...
                    <div className="flex bg-gray-50/80 backdrop-blur-sm">
                        {[
                            ...
                            <button
                                ...
                                disabled={loading}
                                className={`group cursor-pointer flex-1 py-3 px-3 text-xs font-bold transition-all relative ${
                                    filter === tab.key ? "text-main" : "text-gray-600 hover:text-gray-900"
                                }`}
                            >
                                <span className="flex items-center justify-center gap-1.5">
                                    {tab.label}
                                    <span className={`px-2 py-0.5 text-xs rounded-full font-bold transition-all ${
                                        filter === tab.key
