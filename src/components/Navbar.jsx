        ...
        <div className="w-full px-[90px] md:px-[50px] sm:px-2.5 flex justify-center items-center py-5">
            <div className="flex items-center justify-between gap-3">
                <div className="w-20 h-20 max-[1150px]:w-15 max-[1150px]:h-15 max-[900px]:h-12 max-[900px]:w-12  bg-gray-300 border border-[--color-main] rounded-full relative" >
                    ...
                    <img src={flag} className='absolute w-8 border rounded-full h-8 top-[60%] left-[60%]'/>
                </div>
                <div className="text-[--color-main] text-xl  sm:text-xs font-semibold">Eg</div>
                <button className='text-2xl max-[990px]:text-xl md:text-xl sm:text-xs border p-2 rounded-md cursor-pointer text-[--color-main]'>{t('auction')}</button>
                <button className='text-2xl max-[1000px]:text-20 max-[880px]:text-18 sm:text-xs  border p-2 rounded-md cursor-pointer'>{t('shareAdv')}</button>
